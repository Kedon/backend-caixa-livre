...
module.exports.events = async (req, res) => {
  ...
  const { perPage, offset, orderBy, lastId } = req.query
  console.log(req.query)
  try {
    let result = await service.events(userId, perPage, offset, orderBy, lastId);
...
module.exports.details = async (req, res) => {
  ...
  console.log(eventId)
  try {
    let result = await service.details(userId, eventId);
    return res.json(result);
    console.log(result)
  } catch (err) {
    return res.status(500).json(err);
 ...
 * EVENTS INTENTION
 */
module.exports.intention = async (req, res) => {
  const { userId } = req.loggedUser;
  const { eventId } = req.params;
  const { intention, invite } = req.body;
  console.warn(req.body)
  try {
    //VERIFICA SE O USUÁRIO JÁ SE INSCREVEU NO EVENTO
    let subscribed = await service.intentionCheck(userId, eventId);
    let result = (!subscribed) ?
      await service.intentionInsert(userId, eventId, intention, invite)
      :
      await service.intentionUpdate(userId, eventId, intention, invite);
 ...
 * LIST
 */
module.exports.intentionList = async (req, res) => {
  console.log(req)
  const { userId } = req.loggedUser;
  const { eventId, intention } = req.params;
  const { action, offset, lastRow } = req.query
  console.log(req.query)
  try {
    //VERIFICA SE O USUÁRIO JÁ SE INSCREVEU NO EVENTO
    let result =  await service.intentionList(userId, eventId, intention, action, offset, lastRow)
    return res.json(result);
  } catch (err) {
    return res.status(500).json(err);
  }
...
module.exports.invite = async (req, res) => {
  ...
  console.log(req)
  try {
    let result = await service.invite(senderUserId, receiverUserId, eventId);
    return res.json(result);
    console.log(result)
  } catch (err) {
    return res.status(500).json(err);
...
module.exports.uninvite = async (req, res) => {
  ...
  console.log(req)
  try {
    let result = await service.uninvite(senderUserId, receiverUserId, eventId);
    return res.json(result);
  } catch (err) {
    return res.status(500).json(err);
  }
...
module.exports.status = async (req, res) => {
  const receiverUserId = req.loggedUser.userId;
  const senderUserId = req.body.userId;
  const status = req.body.status;
  const eventId = req.params.eventId;
  console.log(status)
  try {
    let result = await service.status(senderUserId, receiverUserId, eventId, status);
    return res.json(result);
  } catch (err) {
    return res.status(500).json(err);
  }
 ...
 * LIST EVENTS
 */
module.exports.invites = async (req, res) => {
  const { userId } = req.loggedUser;
  const { action, offset, lastRow } = req.query
  ...
  try {
    let result = await service.invites(userId, action, offset, lastRow);
    return res.json(result);
    console.log(result)
  } catch (err) {
    return res.status(500).json(err);
